...
- Original scope of this release (extended by [ADR 009](009-profile-management-over-mcp.md)): reads, drafts and adding a posting. The assistant can list and read jobs, add a posting it fetched or was given (`add_job`), fetch a document brief, submit a document, read and edit a revision and render the PDFs. No deletion, no application status change, no profile write. Landed never marks anything applied and never sends anything.
...
- Remote access (claude.ai, a phone) is a separate ADR that reuses this endpoint behind a tunnel with the bearer first and OAuth later. Profile writes from the assistant are specified separately in [ADR 009](009-profile-management-over-mcp.md), including individual-record deletion and the rule about writing facts back.
