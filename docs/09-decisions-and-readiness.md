...
- Interface material and colour (decided 2026-09-14, after Phase 1b): a canvas lit by a glow in the accent's hue, layered glass (flat translucent fill, 1px gradient edge, no blur) on cards, panels and round buttons, Lucide icons in tiles beside every column title, card and sidebar item, icon-only Filter and Sort menus, a light/dark switch, and a palette layer (`src/app/palettes.css`: near-black and near-white neutrals, the accent and glow in one hue; `steel` is the default, `forest`, `ocean` and `dusk` can be tried with `?palette=<name>`). Rules in DESIGN.md.
