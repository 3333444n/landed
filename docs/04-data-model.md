...
## Writing context (implemented and merged)
...
## Companies, findings and Job Sources (implemented and merged)
...
## Document field editing and letterhead (local implementation)

Expanded inline editing continues to store the same validated content JSONB in immutable revisions. Empty optional fields normalize to null; required text stays nonempty. No table or migration change is needed. Cover-letter content adds an optional nullable `title` (maximum 120 characters). Omission projects the frozen profile headline; explicit null hides it. Old JSON remains valid and historical revisions are not rewritten. Metadata edits retain the generation run, evidence and context IDs, and clear approval on the new revision only. Letterhead identity, contact, recipient and date are projected from the revision's frozen snapshot. PDF template version 8 regenerates cached output on demand without modifying saved text or older artifacts.
