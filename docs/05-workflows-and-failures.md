...
Skills and Achievements support Role and Project filters in both the overview and dedicated lists. Filters combine with AND; All leaves that dimension unrestricted, No role/project selects records without that effective context. A role includes its projects' records. Skills combine explicit associations with achievement-derived associations; cards distinguish direct links from derived ones. The URL stores separate `skillsRole`, `skillsProject`, `achievementsRole` and `achievementsProject` parameters. Invalid or removed IDs yield no matches until cleared.
