...
Status: model access path implemented in Phase 1b ([ADR 006](adr/006-model-access-path.md)); the guarded outbound fetch exists for a job's logo address ([ADR 007](adr/007-user-initiated-image-fetch.md), 2026-09-14); the evaluation set has been run against OpenRouter (see "Providers run through the evaluation set"); the assistant surface over MCP is decided and in progress ([ADR 008](adr/008-assistant-surface-over-mcp.md), 2026-09-16; see "Your own assistant over MCP"); agentic research, discovery and retrieval remain future design. Updated 2026-09-16.
...
| `get_document_brief` | `{ job_id, type, assistant?: string }` | Opens a queued `assistant` run with the provider set to the client's `User-Agent` and the model to `assistant` (or `unreported`); returns `{ run_id, instructions, input, schema, budgets, rules }`, the schema being the portable JSON schema without length keywords | not idempotent |
| `submit_document` | `{ run_id, content: object \| string }` | Objects are stringified; `submitPastedAnswer`. Success: `{ revision_id, warnings: [{ kind, path, message }], units }`. Validation failure: the run is recorded as failed (`pasted_invalid`), a fresh queued run is opened, and the result is an error carrying the field errors and `new_run_id` | not destructive |
| `get_document` | `{ job_id, type }` | `getDocumentView`: `{ revision_id, reviewed, units: [{ path, text, evidenceIds }], warnings, latest_run: { mode, state, provider, model } }` | read only |
| `edit_unit` | `{ revision_id, path, text }` | `editUnit` with the revision id as the expected version; returns `{ revision_id }` of the new revision; a stale id is an error that says to call `get_document` again | not destructive |
...
`add_job` (title, company, description, optional location, salary and source URL, with a client-minted id for idempotence, through `pursueJob`) follows in its own pull request; until then the assistant asks the user to paste the posting in the browser. Errors from the modules map to tool errors carrying the error kind and message or field errors; tools never throw. Every tool answers "Create your profile in the browser first" while no profile exists.

What stays as before: the app invoking a harness as a subprocess is not a path (it cannot run inside the release container, and whether a chat subscription may be used that way by a third-party application depends on terms that were not verified). Do not read a user's CLI credential store, and do not assume a chat subscription is an API credential. A local model is not a separate path: Ollama and similar servers are reached as an OpenAI-compatible endpoint, and structured-output quality on small local models is not guaranteed; the evaluation set is the way to find out. A locally running installation is reachable from claude.ai or a phone app only when exposed to the internet with authentication, which documents 03 and 09 keep as a separate design; that design reuses this endpoint.
