...
5. A `letter_length` warning asks you to trim the whole body to three or four sentences and at most 150 words; resubmit if paragraphs must be removed. A `context_number` warning requires reviewing attribution: the number appears only in company/posting context. Verify it describes that company or posting, never the candidate's accomplishments. Do not copy it into career facts to silence the warning; report a justified remaining warning for user review. While other `warnings` remain: fix each one. When the text is the problem (`unsupported_number`, `wraps_line`), find the affected field in `get_document.editable_fields`, then call `edit_unit` with the latest `revision_id`, that field's `path` and the corrected `text`; it keeps the cited ids. A warning may point at a whole entry rather than an editable field. When the evidence ids are the problem (`unknown_evidence`, `no_evidence`, `misattributed_evidence`, `unknown_heading`), correct the ids or the heading in the JSON, call `get_document_brief` again and submit the whole document. Use `get_document.editable_fields` to discover editable paths, labels, current text and `clearable` flags. These include the resume headline, role/project subtitles, skill labels/lists, education fields, cover-letter professional title/greeting/closing/signature, and recruiter-message subject/body, alongside existing prose units. Pass empty `text` only when `clearable` is true; this clears an optional field without removing its entry. Required fields reject blank text. Resume name/contact, work/project names, section titles and entry structure still require a resubmission. An education heading can be edited directly, but it is still checked against frozen facts. A `stale` error means a newer revision exists: call `get_document` and edit that one. Stop when no warnings remain or the user accepts the ones left.
6. For the resume and the cover letter, call `render_pdf` and give the user the `download_url`, the filename and the page count.

For cover letters, aim for three or four body sentences (about 80–150 words), in two or three short paragraphs: a specific connection, one supported career story, and a short close. Consider every supplied company field without forcing it into the prose. Cover-letter `title` is optional (maximum 120 characters): omit to use the frozen profile headline, set null to hide it, or edit the document’s `title` path. Do not invent credentials in a title. The PDF uses a monochrome letterhead with saved sender/contact details, company/role and date above a single-column body, and may leave whitespace. Inline edits affect only that document, create an unapproved revision and preserve its frozen input. Missing optional context is acceptable; do not fill it with guesses. `get_document` returns paragraph `context_ids` alongside `evidence_ids`; submitted JSON uses camelCase keys from the schema. All three generation paths freeze context, so request a fresh brief after context changes.
