...
**Free-text pills.** Desired roles uses removable pills above a labelled text input. Enter or Add commits a trimmed value; comma-separated pastes are supported and duplicates are ignored without regard to case. Each remove control has a named 40px touch target. Saving includes any unfinished entry and validation errors preserve the complete selection.
...
**Company findings.** Companies uses the same URL-driven columns as other sections. The company form has Name, Location, Website and About, followed by a Findings section and its named Add finding toolbar control. Findings use a semantic table with Finding, Source, Retrieved and Kind columns. Finding text opens the edit column; the source hostname links to the supplied URL; Kind visibly distinguishes Statement and Interpretation. The table uses body-sm text, labels for headers, top-aligned wrapping cells, horizontal row separators only, and an overflow wrapper on narrow screens. It sits directly on the detail surface without a surrounding card. Empty state is one secondary-text sentence. Interest provides a searchable multiselect for up to five findings from the linked company, showing each finding's kind and the selected count.

**Job Sources.** Settings has a Job Sources card and list/detail columns for adding, renaming, archiving and restoring named sources. Jobs label the optional searchable selector Source, separate from Posting URL. Both Source and Linked company support an inline text field and explicit create action, followed by selection of the saved option; Cancel returns to the picker. Source starts empty; archived options remain visibly marked on existing assignments and are excluded from new selections. Use the shared Combobox, Field and Button components with persistent helpers and field errors.
