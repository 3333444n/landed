/*
 * Company logos (docs/04): one image file per job under the artifact directory, written before
...
/**
 * Checks and stores an image for a job. `field` names the form field a refusal is reported on
 * (the file input or the address). The file is written to `<profile>/logos/<job>-<hash>.<ext>`.
 */
export async function storeLogo(
  ...
  const storageKey = path.join(
    profileId,
    "logos",
    `${jobId}-${sha256(bytes).slice(0, 8)}.${logoExtensions[contentType]}`,
  );
  await writeFileAtomically(path.join(artifactDir, storageKey), bytes);
  return { ok: true, value: { storageKey, contentType } };
}

/** The stored bytes of a job's logo, or null when the file is missing. */
